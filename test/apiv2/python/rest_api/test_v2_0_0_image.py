...
class ImageTestCase(APITestCase):
    ...
    def test_pull(self):
        ...
        for stream_case in stream_cases:
            ...
            for outcome_case in outcome_cases:
                with self.subTest(stream_case=stream_case, outcome_case=outcome_case):
                    r = requests.post(
                        self.uri(
                            f"/images/pull?reference={outcome_case['reference']}{stream_case['quiet_postfix']}"
                        ),
                        timeout=outcome_case["timeout"],
                    )
                    outcome_case["assert_function"](r.status_code, 200, r.status_code)
                    check_response_keys(r, outcome_case["expected_keys"])
